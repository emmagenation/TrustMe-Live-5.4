...
**Environment:** `PORT`, `DB_FILE`, `ORIGIN` (your site's address), `SHOW_OTP=0` (stop returning the code in the API), `PAYSTACK_SECRET_KEY` (turns on real bank payments and payouts), `APP_URL` (your public address, optional), `MIN_WITHDRAW` (smallest withdrawal in naira, default 1000), `FRONTEND` (the app file to serve, default `TrustMe_1_1_2.html`), `VAPID_PUBLIC` + `VAPID_PRIVATE` (the keys that sign push alerts, see **Notifications**), `VAPID_SUBJECT` (a `mailto:` or `https:` contact for the push services, default `APP_URL` or a placeholder: set it).
