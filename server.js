...
const FRONTEND = path.join(__dirname, process.env.FRONTEND || 'TrustMe_1_1_3.html');

let db = { users: {}, sessions: {}, otps: {}, bookings: {}, messages: [], ledger: [], payouts: {}, notifs: [], push: {}, seq: 0 };
...
/* Dispute replies: disputes are still kept on the device only, so nothing fires yet. When a dispute (or a support reply to one) is
   saved on the server, call notify(corperId, 'dispute', 'Dispute update', 'We replied to your dispute about ' + b.what + '.', 'booking:' + b.id). */
...
const routes = [
  ...
  ['POST', /^\/api\/bookings\/(\w+)\/cancel$/, async c => {           // the corps member backs out before the vendor has accepted
    if (c.u.role !== 'corper') fail(403, 'Only corps members can cancel a booking.');
